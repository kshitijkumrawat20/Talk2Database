...
def init_history_db():
    try:
        ...
        conn.commit()
        conn.close()
        print("Query history table initialized successfully.")
    except Exception as e:
        print(f"Error initializing query_history table: {e}")
...
def add_connection(name:str, connection_strings:str, username: str):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO connections (name, connection_strings, username) VALUES (?, ?, ?)", (name, connection_strings, username))
    conn.commit()
